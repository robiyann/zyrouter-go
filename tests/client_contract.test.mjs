...
assert.match(app, /EventSource\(streamUrl, \{ withCredentials: true \}\)/, 'SSE must send the HttpOnly user cookie');
...
assert.match(app, /const INFERENCE_BASE/, 'client must define a separate inference API base');
assert.match(html, /api\.zyvenox\.tech\/v1/, 'client API guide must point to the inference domain');
