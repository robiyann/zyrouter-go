...
func SetupRoutes(r interface {
	...
	r.Get("/api/quota", adminH.HandleGetQuota)
	r.Get("/api/admin/quota", adminH.HandleGetQuota)
	r.Post("/api/quota/settings", adminH.HandleUpdateQuotaSettings)
	r.Post("/api/admin/quota/settings", adminH.HandleUpdateQuotaSettings)
